...
public sealed class CacheMonitor : DisposableMediatorSubscriberBase
{
    ...
    public CacheMonitor(ILogger<CacheMonitor> logger, IpcManager ipcManager, MoonlightConfigService configService,
        FileCacheManager fileDbManager, MoonlightMediator mediator, PerformanceCollectorService performanceCollector, DalamudUtilService dalamudUtil,
        FileCompactor fileCompactor) : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<PenumbraInitializedMessage>(this, (_) =>
        {
            ...
            StartMoonlightWatcher(configService.Current.CacheFolder);
            InvokeScan();
        });
    ...
    private readonly Dictionary<string, WatcherChange> _moonlightChanges = new Dictionary<string, WatcherChange>(StringComparer.OrdinalIgnoreCase);

    public void StopMonitoring()
    {
        Logger.LogInformation("Stopping monitoring of Penumbra and Moonlight storage folders");
        MoonlightWatcher?.Dispose();
        PenumbraWatcher?.Dispose();
        MoonlightWatcher = null;
        PenumbraWatcher = null;
    }

    public bool StorageisNTFS { get; private set; } = false;

    public void StartMoonlightWatcher(string? MoonlightPath)
    {
        MoonlightWatcher?.Dispose();
        if (string.IsNullOrEmpty(MoonlightPath) || !Directory.Exists(MoonlightPath))
        {
            MoonlightWatcher = null;
            Logger.LogWarning("Moonlight file path is not set, cannot start the FSW for Moonlight.");
            return;
        }
        ...
        Logger.LogInformation("Moonlight Storage is on NTFS drive: {isNtfs}", StorageisNTFS);

        Logger.LogDebug("Initializing Moonlight FSW on {path}", MoonlightPath);
        MoonlightWatcher = new()
        {
            Path = MoonlightPath,
        ...
        MoonlightWatcher.Deleted += MoonlightWatcherFileChanged;
        MoonlightWatcher.Created += MoonlightWatcherFileChanged;
        MoonlightWatcher.EnableRaisingEvents = true;
    }

    private void MoonlightWatcherFileChanged(object sender, FileSystemEventArgs e)
    {
        Logger.LogTrace("Moonlight FSW: FileChanged: {change} => {path}", e.ChangeType, e.FullPath);

        if (!AllowedFileExtensions.Any(ext => e.FullPath.EndsWith(ext, StringComparison.OrdinalIgnoreCase))) return;

        lock (_watcherChanges)
        {
            _moonlightChanges[e.FullPath] = new(e.ChangeType);
        }

        _ = MoonlightWatcherExecution();
    }
    ...
    private CancellationTokenSource _moonlightFswCts = new();
    public FileSystemWatcher? PenumbraWatcher { get; private set; }
    public FileSystemWatcher? MoonlightWatcher { get; private set; }

    private async Task MoonlightWatcherExecution()
    {
        _moonlightFswCts = _moonlightFswCts.CancelRecreate();
        var token = _moonlightFswCts.Token;
        var delay = TimeSpan.FromSeconds(5);
        Dictionary<string, WatcherChange> changes;
        lock (_moonlightChanges)
            changes = _moonlightChanges.ToDictionary(t => t.Key, t => t.Value, StringComparer.Ordinal);
        try
        {
            do
            {
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        _moonlightFswCts?.CancelDispose();
    ...
    private void FullFileScan(CancellationToken ct)
    {
        ...
        if (string.IsNullOrEmpty(_configService.Current.CacheFolder) || !Directory.Exists(_configService.Current.CacheFolder))
        {
            ...
            Logger.LogWarning("Moonlight Cache directory is not set or does not exist.");
        }
        ...
        if (!_configService.Current.InitialScanComplete)
        {
            ...
            _configService.Save();
            StartMoonlightWatcher(_configService.Current.CacheFolder);
