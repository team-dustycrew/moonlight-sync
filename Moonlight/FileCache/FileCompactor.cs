...
public sealed class FileCompactor
{
    ...
    public FileCompactor(ILogger<FileCompactor> logger, MoonlightConfigService moonlightConfigService, DalamudUtilService dalamudUtilService)
    {
    ...
    public void CompactStorage(bool compress)
    {
        ...
        int currentFile = 1;
        var allFiles = Directory.EnumerateFiles(_moonlightConfigService.Current.CacheFolder).ToList();
    ...
    public async Task WriteAllBytesAsync(string filePath, byte[] decompressedFile, CancellationToken token)
    {
        ...
        if (_dalamudUtilService.IsWine || !_moonlightConfigService.Current.UseCompactor)
        {
            return;
        }
