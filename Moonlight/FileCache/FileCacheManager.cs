...
public sealed class FileCacheManager : IHostedService
{
    ...
    public FileCacheManager(ILogger<FileCacheManager> logger, IpcManager ipcManager, MoonlightConfigService configService, MoonlightMediator moonlightMediator)
    {
        _logger = logger;
    ...
    {
        _moonlightMediator.Publish(new HaltScanMessage(nameof(ValidateLocalIntegrity)));
        ...
        _moonlightMediator.Publish(new ResumeScanMessage(nameof(ValidateLocalIntegrity)));
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        if (File.Exists(_csvPath))
        {
            ...
            {
                _moonlightMediator.Publish(new NotificationMessage("Penumbra not connected",
                    "Could not load local file cache data. Penumbra is not connected or not properly set up. Please enable and/or configure Penumbra properly to use Moonlight. After, reload Moonlight in the Plugin installer.",
                    MoonlightConfiguration.Models.NotificationType.Error));
            }
