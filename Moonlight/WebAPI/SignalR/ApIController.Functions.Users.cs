...
public partial class ApiController
{
    ...
    {
        if (!IsConnected) return;
        await _moonlightHub!.SendAsync(nameof(UserAddPair), user).ConfigureAwait(false);
    }

    public async Task UserDelete()
    {
        CheckConnection();
        await _moonlightHub!.SendAsync(nameof(UserDelete)).ConfigureAwait(false);
    ...
    {
        return await _moonlightHub!.InvokeAsync<List<OnlineUserIdentDto>>(nameof(UserGetOnlinePairs), censusDataDto).ConfigureAwait(false);
    }

    public async Task<List<UserFullPairDto>> UserGetPairedClients()
    {
        return await _moonlightHub!.InvokeAsync<List<UserFullPairDto>>(nameof(UserGetPairedClients)).ConfigureAwait(false);
    }
    ...
    public async Task<UserProfileDto> UserGetProfile(UserDto dto)
    {
        ...
        return await _moonlightHub!.InvokeAsync<UserProfileDto>(nameof(UserGetProfile), dto).ConfigureAwait(false);
    }
    ...
    public async Task UserPushData(UserCharaDataMessageDto dto)
    {
        try
        {
            await _moonlightHub!.InvokeAsync(nameof(UserPushData), dto).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    {
        if (!IsConnected) return;
        await _moonlightHub!.SendAsync(nameof(UserRemovePair), userDto).ConfigureAwait(false);
    }
    ...
    {
        if (!IsConnected) return;
        await _moonlightHub!.InvokeAsync(nameof(UserSetProfile), userDescription).ConfigureAwait(false);
    }

    public async Task UserUpdateDefaultPermissions(DefaultPermissionsDto defaultPermissionsDto)
    {
        CheckConnection();
        await _moonlightHub!.InvokeAsync(nameof(UserUpdateDefaultPermissions), defaultPermissionsDto).ConfigureAwait(false);
    }
