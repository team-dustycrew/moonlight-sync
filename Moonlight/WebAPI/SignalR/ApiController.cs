...
public sealed partial class ApiController : DisposableMediatorSubscriberBase, IMoonLightHubClient
{
    ...
    public ApiController(ILogger<ApiController> logger, HubFactory hubFactory, DalamudUtilService dalamudUtil,
        PairManager pairManager, ServerConfigurationManager serverManager, MoonlightMediator mediator,
        TokenProvider tokenProvider, MoonlightConfigService moonlightConfigService) : base(logger, mediator)
    {
    ...
    public async Task CreateConnectionsAsync()
    {
        ...
        if (!_serverManager.CurrentServer.UseOAuth2)
        {
            ...
            if (multi)
            {
                ...
                _connectionDto = null;
                Mediator.Publish(new NotificationMessage("Multiple Identical Characters detected", "Your Service configuration has multiple characters with the same name and world set up. Delete the duplicates in the character management to be able to connect to Moonlight.",
