...
public sealed class IpcCallerPetNames : IIpcCaller
{
    ...
    {
        CheckAPI();
        _moonlightMediator.Publish(new PetNamesReadyMessage());
    }

    private void OnPetNicknamesDispose()
    {
        _moonlightMediator.Publish(new PetNamesMessage(string.Empty));
    }
    ...
    {
        _moonlightMediator.Publish(new PetNamesMessage(data));
    }
