...
internal partial class CharaDataHubUi
{
    private void DrawNearbyPoses()
    {
        ...
        UiSharedService.DrawTree("Settings & Filters", () =>
        {
            ...
            _uiSharedService.DrawHelpText("When enabled, Moonlight will draw floating wisps where other's poses are in the world.");
            ...
            _uiSharedService.DrawHelpText("This will allow Moonlight to continue the calculation of position of wisps etc. active outside of the 'Poses Nearby' tab." + UiSharedService.TooltipSeparator
        ...
        foreach (var pose in _charaDataNearbyManager.NearbyData.OrderBy(v => v.Value.Distance))
        {
            ...
            {
                string? userNote = _serverConfigurationManager.GetNoteForUid(new Guid(pose.Key.MetaInfo.Uploader.publicUserID));
