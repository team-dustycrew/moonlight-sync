...
public class IdDisplayHandler
{
    ...
    public IdDisplayHandler(MoonlightMediator mediator, ServerConfigurationManager serverManager, MoonlightConfigService moonlightConfigService)
    {
        _mediator = mediator;
    ...
    public void DrawPairText(string id, Pair pair, float textPosX, Func<float> editBoxWidth)
    {
        ...
        if (!string.Equals(_editEntry, pair.UserData.UID, StringComparison.Ordinal))
        {
            ...
            if (ImGui.IsItemHovered())
            {
                ...
                {
                    _popupTime = DateTime.UtcNow.AddSeconds(_moonlightConfigService.Current.ProfileDelay);
                }

                _lastMouseOverUid = id;

                if (_popupTime > DateTime.UtcNow || !_moonlightConfigService.Current.ProfilesShow)
                {
    ...
    public (bool isUid, string text) GetPlayerText(Pair pair)
    {
        ...
        if (_moonlightConfigService.Current.ShowCharacterNameInsteadOfNotesForVisible && pair.IsVisible && !showUidInsteadOfName)
        {
            ...
            textIsUid = false;
            if (_moonlightConfigService.Current.PreferNotesOverNamesForVisible)
            {
