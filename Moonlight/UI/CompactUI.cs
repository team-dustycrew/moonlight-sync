...
public class CompactUi : WindowMediatorSubscriberBase
{
    ...
    public CompactUi(ILogger<CompactUi> logger, UiSharedService uiShared, MoonlightConfigService configService, ApiController apiController, PairManager pairManager,
        ServerConfigurationManager serverManager, MoonlightMediator mediator, FileUploadManager fileTransferManager,
        TagHandler tagHandler, DrawEntityFactory drawEntityFactory, SelectTagForPairUi selectTagForPairUi, SelectPairForTagUi selectPairForTagUi,
        PerformanceCollectorService performanceCollectorService, IpcManager ipcManager)
        : base(logger, mediator, "###MoonlightMainUI", performanceCollectorService)
    {
        ...
        TitleBarButtons = new()
        {
            new Window.TitleBarButton()
            {
                ...
                {
                    ImGui.BeginTooltip();
                    ImGui.Text("Open Moonlight Settings");
                    ImGui.EndTooltip();
                }
            },
            new Window.TitleBarButton()
            {
                ...
                {
                    ImGui.BeginTooltip();
                    ImGui.Text("Open Moonlight Event Viewer");
                    ImGui.EndTooltip();
                }
            }
        };
        ...
        WindowName = $"Moonlight {dev} ({ver.Major}.{ver.Minor}.{ver.Build})###MoonlightMainUI";
        Toggle();
#else
        var ver = Assembly.GetExecutingAssembly().GetName().Version;
        WindowName = "Moonlight  " + ver.Major + "." + ver.Minor + "." + ver.Build + "###MoonlightMainUI";
#endif
    ...
    protected override void DrawInternal()
    {
        ...
        if (!_apiController.IsCurrentVersion)
        {
            ...
            UiSharedService.ColorTextWrapped($"Your Moonlight installation is out of date, the current version is {ver.Major}.{ver.Minor}.{ver.Build}. " +
                $"It is highly recommended to keep Moonlight up to date. Open /xlplugins and update the plugin.", ImGuiColors.DalamudRed);
        }
        ...
        if (!_ipcManager.Initialized)
        {
            ...
            UiSharedService.ColorTextWrapped($"One or more Plugins essential for Moonlight operation are unavailable. Enable or update following plugins:", ImGuiColors.DalamudRed);
    ...
    private string GetServerError()
    {
        return _apiController.ServerState switch
        {
            ...
            ServerState.Disconnected => "You are currently disconnected from the Moonlight server.",
            ServerState.Disconnecting => "Disconnecting from the server",
            ServerState.Unauthorized => "Server Response: " + _apiController.AuthFailureMessage,
            ServerState.Offline => "Your selected Moonlight server is currently offline.",
