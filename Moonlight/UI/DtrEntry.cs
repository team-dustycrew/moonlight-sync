...
public sealed class DtrEntry : IDisposable, IHostedService
{
    ...
    private readonly ConfigurationServiceBase<MoonlightConfig> _configService;
    ...
    public DtrEntry(ILogger<DtrEntry> logger, IDtrBar dtrBar, ConfigurationServiceBase<MoonlightConfig> configService, MoonlightMediator moonlightMediator, PairManager pairManager, ApiController apiController)
    {
        _logger = logger;
        _dtrBar = dtrBar;
    ...
    private IDtrBarEntry CreateEntry()
    {
        ...
        var entry = _dtrBar.Get("Moonlight");
        entry.OnClick = _ => _moonlightMediator.Publish(new UiToggleMessage(typeof(CompactUi)));
    ...
    private void Update()
    {
        ...
        if (_apiController.IsConnected)
        {
            ...
            if (pairCount > 0)
            {
                ...
                tooltip = $"Moonlight: Connected{Environment.NewLine}----------{Environment.NewLine}{string.Join(Environment.NewLine, visiblePairs)}";
                colors = _configService.Current.DtrColorsPairsInRange;
            }
            else
            {
                tooltip = "Moonlight: Connected";
        ...
        {
            text = "\uE044 \uE04C";
            tooltip = "Moonlight: Not Connected";
