...
public class TopTabMenu
{
    ...
    public TopTabMenu(MoonlightMediator moonlightMediator, ApiController apiController, PairManager pairManager, UiSharedService uiSharedService)
    {
    ...
    private void DrawSyncshellMenu(float availableWidth, float spacingX)
    {
            ...
            .Count(g => string.Equals(g.OwnerUID.ToString(), _apiController.UID.ToString(), StringComparison.Ordinal)) >= _apiController.ServerInfo.MaxGroupsCreatedByUser))
        {
            ...
            {
                _moonlightMediator.Publish(new UiToggleMessage(typeof(CreateSyncshellUI)));
            }
            ImGui.SameLine();
        }
        ...
        using (ImRaii.Disabled(_pairManager.GroupPairs.Select(k => k.Key).Distinct().Count() >= _apiController.ServerInfo.MaxGroupsJoinedByUser))
        {
            ...
            {
                _moonlightMediator.Publish(new UiToggleMessage(typeof(JoinSyncshellUI)));
            }
        }
    }
    ...
    {
        var buttonX = (availableWidth - spacingX) / 2f;
        if (_uiSharedService.IconTextButton(FontAwesomeIcon.UserCircle, "Edit Moonlight Profile", buttonX))
        {
            _moonlightMediator.Publish(new UiToggleMessage(typeof(EditProfileUi)));
        }
        UiSharedService.AttachToolTip("Edit your Moonlight Profile");
        ImGui.SameLine();
        if (_uiSharedService.IconTextButton(FontAwesomeIcon.PersonCircleQuestion, "Chara Data Analysis", buttonX))
        {
            _moonlightMediator.Publish(new UiToggleMessage(typeof(DataAnalysisUi)));
        }
        ...
        {
            _moonlightMediator.Publish(new UiToggleMessage(typeof(CharaDataHubUi)));
        }
    }
