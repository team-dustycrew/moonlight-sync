...
public partial class UiSharedService : DisposableMediatorSubscriberBase
{
    ...
    private const string _notesEnd = "##MOONLIGHT_USER_NOTES_END##";
    private const string _notesStart = "##MOONLIGHT_USER_NOTES_START##";
    ...
    public UiSharedService(ILogger<UiSharedService> logger, IpcManager ipcManager, ApiController apiController,
        ...
        MoonlightConfigService configService, DalamudUtilService dalamudUtil, IDalamudPluginInterface pluginInterface,
        ITextureProvider textureProvider,
        Dalamud.Localization localization,
        ServerConfigurationManager serverManager, TokenProvider tokenProvider, MoonlightMediator mediator) : base(logger, mediator)
    {
    ...
    {
        ColorTextWrapped("Note: The storage folder should be somewhere close to root (i.e. C:\\MoonlightStorage) in a new empty folder. DO NOT point this to your game folder. DO NOT point this to your Penumbra folder.", ImGuiColors.DalamudYellow);
        ...
        ImGui.SameLine();
        using (ImRaii.Disabled(_cacheMonitor.MoonlightWatcher != null))
        {
            if (IconButton(FontAwesomeIcon.Folder))
            {
                FileDialogManager.OpenFolderDialog("Pick Moonlight Storage Folder", (success, path) =>
                {
                    if (!success) return;
                    ...
                    {
                        _cacheDirectoryHasOtherFilesThanCache = true;
                        Logger.LogWarning("Found folders in {path} not belonging to Moonlight: {dirs}", path, string.Join(", ", dirs));
                    }
                        ...
                        && _cacheDirectoryIsValidPath)
                    {
                        ...
                        _configService.Save();
                        _cacheMonitor.StartMoonlightWatcher(path);
                        _cacheMonitor.InvokeScan();
                    }
                }, _dalamudUtil.IsWine ? @"Z:\" : @"C:\");
            }
        }
        if (_cacheMonitor.MoonlightWatcher != null)
        {
        ...
        {
            ColorTextWrapped("Your selected directory has files or directories inside that are not Moonlight related. Use an empty directory or a previous Moonlight storage directory only.", ImGuiColors.DalamudRed);
        }
        ...
        DrawHelpText("The storage is automatically governed by Moonlight. It will clear itself automatically once it reaches the set capacity by removing the oldest unused files. You typically do not need to clear it yourself.");
    }
    ...
    public bool DrawOtherPluginState()
    {
        ...
        {
            ImGui.TextColored(ImGuiColors.DalamudRed, "You need to install both Penumbra and Glamourer and keep them up to date to use Moonlight.");
            return false;
        }
