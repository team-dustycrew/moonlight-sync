...
public sealed class PairHandler : DisposableMediatorSubscriberBase
{
    ...
    private readonly Dictionary<Enum_ObjectKind, Guid?> _customizeIds = [];
    ...
    public PairHandler(ILogger<PairHandler> logger, Pair pair,
        ...
        FileCacheManager fileDbManager, MoonlightMediator mediator,
    ...
    public void ApplyCharacterData(Guid applicationBase, CharacterData characterData, bool forceApplyCustomization = false)
    {
        ...
        if (_redrawOnNextApplication && charaDataToUpdate.TryGetValue(Enum_ObjectKind.Player, out var player))
        {
        ...
        try
        {
            ...
            if (_dalamudUtil is { IsZoning: false, IsInCutscene: false } && !string.IsNullOrEmpty(name))
            {
                ...
                else
                {
                    ...
                    foreach (KeyValuePair<Enum_ObjectKind, List<FileReplacementData>> item in _cachedData?.FileReplacements ?? [])
                    {
                        try
                        {
    ...
    private async Task ApplyCustomizationDataAsync(Guid applicationId, KeyValuePair<Enum_ObjectKind, HashSet<PlayerChanges>> changes, CharacterData charaData, CancellationToken token)
    {
    ...
    private void DownloadAndApplyCharacter(Guid applicationBase, CharacterData charaData, Dictionary<Enum_ObjectKind, HashSet<PlayerChanges>> updatedData)
    {
        if (!updatedData.Any())
        {
    ...
    private async Task DownloadAndApplyCharacterAsync(Guid applicationBase, CharacterData charaData, Dictionary<Enum_ObjectKind, HashSet<PlayerChanges>> updatedData,
    ...
    private async Task ApplyCharacterDataAsync(Guid applicationBase, CharacterData charaData, Dictionary<Enum_ObjectKind, HashSet<PlayerChanges>> updatedData, bool updateModdedPaths, bool updateManip,
    ...
    private async Task RevertCustomizationDataAsync(Enum_ObjectKind objectKind, string name, Guid applicationId, CancellationToken cancelToken)
    {
