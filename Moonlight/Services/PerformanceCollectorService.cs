...
public sealed class PerformanceCollectorService : IHostedService
{
    ...
    public PerformanceCollectorService(ILogger<PerformanceCollectorService> logger, MoonlightConfigService moonlightConfigService)
    {
        _logger = logger;
        _moonlightConfigService = moonlightConfigService;
    }

    public T LogPerformance<T>(object sender, MoonlightInterpolatedStringHandler counterName, Func<T> func, int maxEntries = 10000)
    {
        if (!_moonlightConfigService.Current.LogPerformance) return func.Invoke();
    ...
    public void LogPerformance(object sender, MoonlightInterpolatedStringHandler counterName, Action act, int maxEntries = 10000)
    {
        if (!_moonlightConfigService.Current.LogPerformance) { act.Invoke(); return; }
    ...
    {
        if (!_moonlightConfigService.Current.LogPerformance)
        {
