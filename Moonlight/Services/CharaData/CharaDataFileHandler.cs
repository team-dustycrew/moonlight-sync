...
public sealed class CharaDataFileHandler : IDisposable
{
    ...
    private readonly MoonlightCharaFileDataFactory _moonlightCharaFileDataFactory;
            ...
            DalamudUtilService dalamudUtilService, GameObjectHandlerFactory gameObjectHandlerFactory, PlayerDataFactory playerDataFactory)
    {
        ...
        _moonlightCharaFileDataFactory = new(fileCacheManager);
    }
    ...
    public Task<(MoonlightCharaFileHeader loadedCharaFile, long expectedLength)> LoadCharaFileHeader(string filePath)
    {
        try
        {
            ...
            var loadedCharaFile = MoonlightCharaFileHeader.FromBinaryReader(filePath, reader);

            _logger.LogInformation("Read Moonlight Chara File");
    ...
    public Dictionary<string, string> McdfExtractFiles(MoonlightCharaFileHeader? charaFileHeader, long expectedLength, List<string> extractedFiles)
    {
        ...
        using var reader = new BinaryReader(lz4Stream);
        MoonlightCharaFileHeader.AdvanceReaderToData(reader);

        long totalRead = 0;
        Dictionary<string, string> gamePathToFilePath = new(StringComparer.Ordinal);
        foreach (var fileData in charaFileHeader.CharaFileData.Files)
        {
            var fileName = Path.Combine(_fileCacheManager.CacheFolder, "Moonlight_" + _globalFileCounter++ + ".tmp");
    ...
    internal async Task SaveCharaFileAsync(string description, string filePath)
    {
        ...
        try
        {
            ...
            var MoonlightCharaFileData = _moonlightCharaFileDataFactory.Create(description, data);
            MoonlightCharaFileHeader output = new(MoonlightCharaFileHeader.CurrentVersion, MoonlightCharaFileData);
        ...
        {
            _logger.LogError(ex, "Failure Saving Moonlight Chara File, deleting output");
