...
public sealed class CharaDataNearbyManager : DisposableMediatorSubscriberBase
{
    ...
    public CharaDataNearbyManager(ILogger<CharaDataNearbyManager> logger, MoonlightMediator mediator,
    ...
    private async Task FilterEntriesAsync(Vector3 cameraPos, Vector3 cameraLookAt)
    {
        ...
        foreach (var data in _metaInfoCache.Where(d => (string.IsNullOrWhiteSpace(UserNoteFilter)
            ...
            || d.Key.AliasOrUID.Contains(UserNoteFilter, StringComparison.OrdinalIgnoreCase)
            || (_serverConfigurationManager.GetNoteForUid(new Guid(d.Key.publicUserID)) ?? string.Empty).Contains(UserNoteFilter, StringComparison.OrdinalIgnoreCase))))
